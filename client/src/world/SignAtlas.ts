...
export class SignAtlas {
  ...
  /** @param origin where the mesh sits; signs keep their world positions, stored relative to it for precision. */
  constructor(signs: readonly AtlasSign[], origin: { x: number; y: number; z: number } = { x: 0, y: 0, z: 0 }) {
    ...
    signs.forEach((sign, index) => {
      ...
      corners.forEach(([cx, cy], corner) => {
        ...
        positions[at] = sign.x - origin.x + cx * cos;
        positions[at + 1] = sign.y - origin.y + cy;
        positions[at + 2] = sign.z - origin.z - cx * sin;
      });
