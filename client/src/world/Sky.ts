...
import { worldAtX, worldOriginX } from '@highjump/shared';
...
/**
 * Height of one repeating band of clouds. The staircase climbs past a hundred
 * thousand units, so instead of strewing clouds up all of it, one band tiles
 * vertically around the camera: three copies, re-seated a whole band at a time,
 * which moves nothing visible.
 */
const CLOUD_BAND = 2400;
...
/**
 * A gradient dome that FOLLOWS the camera, plus a band of blocky clouds that
 * tiles up (and down) the sky with it, so every altitude of the climb has clouds
 * around it. Four draw calls in all.
 */
export class Sky {
  readonly root = new Group();
  private readonly dome: Mesh;
  private readonly cloudLayers: Mesh[] = [];
  ...
  constructor(courseLength: number) {
    ...
    this.buildClouds(courseLength);
  }

  follow(x: number, y: number, z: number): void {
    this.dome.position.set(x, y, z);
    const base = Math.floor(y / CLOUD_BAND) * CLOUD_BAND;
    // The clouds line whichever world's staircase the camera is at.
    const originX = worldOriginX(worldAtX(x));
    this.cloudLayers.forEach((layer, i) => {
      layer.position.set(originX, base + (i - 1) * CLOUD_BAND, 0);
    });
  }

  private buildClouds(courseLength: number): void {
    ...
    for (let i = 0; i < clusters; i += 1) {
      ...
      const cy = 40 + random() * (CLOUD_BAND - 80);
      const scale = 10 + random() * 16;
      const blocks = 4 + Math.floor(random() * 4);
      for (let b = 0; b < blocks; b += 1) {
        const t = blocks === 1 ? 0.5 : b / (blocks - 1);
        const bulge = Math.sin(t * Math.PI);
        const box = new BoxGeometry(scale * (1.1 + bulge * 1.5), scale * (0.5 + bulge * 0.55), scale * (1 + bulge * 1.2));
