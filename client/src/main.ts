...
const main = async (): Promise<void> => {
  ...
  if (clientConfig.debug) {
    (window as Window & { __balloon?: { game: Game; loop: GameLoop } }).__balloon = { game, loop };
  }
  ...
  if (bootStatus) {
    ...
    bootStatus.textContent = clientConfig.serverUrl
      ? `Not connected to the game server (${clientConfig.serverUrl}).\n` +
        'Playing offline: balloons, Wins, pets and shops are server-owned and will not progress.'
