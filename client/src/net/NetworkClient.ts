...
const PLAYER_ID_KEY = 'balloonpersecond.playerId';
...
export class NetworkClient {
  ...
  claimWin(area: number): void {
    const message: ClaimWinMessage = { area };
  ...
  /** A request that carries nothing: Equip Best. */
  ...
  get leaderboard(): LeaderboardSnapshot | null {
    ...
    return { balloons: copy(board.balloons), wins: copy(board.wins), time: copy(board.time) };
  }
